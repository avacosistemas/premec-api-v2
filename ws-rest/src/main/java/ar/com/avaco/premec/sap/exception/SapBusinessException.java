...
public class SapBusinessException extends Exception {
    ...
    public static final Integer ERRROR_NO_MATCHING_RECORDS_FOUND = -2028;
    ...
    public SapBusinessException(String message, Throwable cause) {
        super(message, cause);
    }
     ...
     * =============================== */
    public SapBusinessException(
            SapErrorResponseDTO sapError,
            int httpStatus) {

        super(sapError.getError().getMessage());

        this.sapCode = Integer.valueOf(sapError.getError().getCode());
        this.sapMessage = sapError.getError().getMessage();
        this.httpStatus = httpStatus;

        // compatibilidad con lógica existente
        this.errorCode = this.sapCode;
    }
    ...
    /* ===============================
     * Lógica legacy (no se elimina)
     * =============================== */
    private int getErrorCode(RestClientException rce) {
        try {
            return ((Double) ((LinkedTreeMap<?, ?>)
                    ((LinkedTreeMap<?, ?>)
                            new Gson().fromJson(rce.getMessage(), Object.class))
                            .get("error"))
                            .get("code"))
                    .intValue();
        } catch (Exception e) {
